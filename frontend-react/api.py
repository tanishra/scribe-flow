...
async def generate_blog_task_streaming(job_id: str, topic: str, tone: str):
    ...
    async with generation_semaphore:
        ...
        try:
            ...
            # We need at least the plan and SEO/final content to consider it a success
            if final_output.get("plan") or final_output.get("final"):
                ...
                final_md = final_output.get("final", "")
                
                # Fallback for Title if plan is missing from this event but exists in DB
                ...
                # FINAL FALLBACK: Check disk if the file was written by the node itself
                # This handles cases where the streamer loop finishes before catching the last event
                with next(get_session()) as session:
                    db_blog = session.exec(select(Blog).where(Blog.job_id == job_id)).first()
                    # If we have a title from DB, we can check disk
                    if db_blog and db_blog.topic:
                        # We don't have the final title yet possibly, but let's check if finalize_blog was logged
                        logger.info("Stream ended without full output capture. Checking DB/Disk state...")
                        # If finalize_blog node finishes, it usually means success
                        # For now, let's just mark completed if we got some content
