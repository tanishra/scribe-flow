...
import { User, Mail, Camera, Save, ArrowLeft, ShieldCheck, Zap, Coins, Globe, Key } from 'lucide-react';
  ...
  useEffect(() => {
    if (user) {
      setData({
        ...
        source: user.source || '',
        bio: user.bio || '',
        devto_api_key: user.devto_api_key || '', // NEW
      });
    }
  }, [user]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-6">
          <GlassCard className="text-center p-8 text-left">
            <div className="relative inline-block group">
              ...
              <button 
                ...
                className="absolute bottom-2 right-2 p-2 bg-blue-600 rounded-full text-white shadow-lg hover:bg-blue-500 transition-all opacity-0 group-hover:opacity-100"
              >
          ...
          <GlassCard className="p-6 text-left">
            <div className="flex items-center gap-3 text-green-400 text-sm font-semibold mb-4">
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-3 text-slate-300">
                    ...
                    <span className="text-sm truncate" title={user.email}>{user.email || 'No email'}</span>
                </div>
            </div>
          </GlassCard>
        </div>
          ...
          <GlassCard className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="space-y-2">
                ...
                <input value={data.full_name} onChange={e => setData({...data, full_name: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Profession</label>
                <input value={data.profession} onChange={e => setData({...data, profession: e.target.value})} className="w-full bg-black/20 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50" />
              </div>
            </div>
            <div className="space-y-2 mb-8">
              <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Bio / Description</label>
              <textarea value={data.bio} onChange={e => setData({...data, bio: e.target.value})} rows={3} placeholder="Tell us a little about yourself..." className="w-full bg-black/20 border border-white/10 rounded-xl p-4 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 resize-none" />
            </div>

            <h3 className="text-2xl font-bold text-white mb-8 border-b border-white/5 pb-4 flex items-center gap-3">
                ...
                <div className="p-6 rounded-2xl bg-white/5 border border-white/10">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-black rounded-lg flex items-center justify-center font-bold text-white text-xs border border-white/10">DEV</div>
                            <div>
                                <h4 className="font-bold text-white">Dev.to</h4>
                                <p className="text-[10px] text-slate-500 uppercase font-black">Publish drafts to Dev.to community</p>
                            </div>
                        </div>
                    ...
                    <div className="relative">
                        ...
                        <input 
                            ...
                            value={data.devto_api_key}
                            onChange={e => setData({...data, devto_api_key: e.target.value})}
